...
export async function renderSafeMarkdown(markdown) {
  const file = await unified()
    ...
    .use(() => (tree) => visit(tree, 'heading', (node) => { if (node.depth <= 1) node.depth = 2; }))  // demote h1→h2 (template already renders title as h1)
    .use(remarkRehype, { allowDangerousHtml: true })  // pass raw HTML divs through (sanitize handles safety)
